...
ax1.set_title("Bus Voltage Magnitudes (pu) — OPF Economic Dispatch\n"
              "Colour: green = nominal, red = near/outside ±5% limit",
              fontsize=11, fontweight='bold')

cbar1 = fig1.colorbar(im1, ax=ax1, fraction=0.025, pad=0.02)
cbar1.set_label('Voltage (pu)', fontsize=9)
cbar1.ax.axhline(y=1.05, color='red', linewidth=1.5, linestyle='--')
cbar1.ax.axhline(y=0.95, color='red', linewidth=1.5, linestyle='--')

plt.tight_layout()
plt.savefig(os.path.join(script_dir, 'task1_voltage_heatmap.png'),
            dpi=150, bbox_inches='tight')
print("[Saved] task1_voltage_heatmap.png")
...
norm_l = mcolors.Normalize(vmin=0, vmax=110)
...
for row in range(n_rows):
    for col in range(n_lines):
        if np.isnan(ll_matrix[row, col]):
            ax2.text(col, row, 'n/c', ha='center', va='center',
...
ax2.set_xticks(range(n_lines))
ax2.set_xticklabels([f'Line {i}' for i in range(n_lines)], fontsize=9)
ax2.set_yticks(range(n_rows))
ax2.set_yticklabels(row_labels, fontsize=8)
ax2.set_xlabel("Line", fontsize=10)
ax2.set_title("Line Loading (%) — OPF Economic Dispatch\n"
              "Colour: white/yellow = low load, red = near thermal limit",
              fontsize=11, fontweight='bold')

cbar2 = fig2.colorbar(im2, ax=ax2, fraction=0.03, pad=0.02)
cbar2.set_label('Loading (%)', fontsize=9)
cbar2.ax.axhline(y=100, color='red', linewidth=2, linestyle='--')

plt.tight_layout()
plt.savefig(os.path.join(script_dir, 'task1_lineloading_heatmap.png'),
            dpi=150, bbox_inches='tight')
print("[Saved] task1_lineloading_heatmap.png")
...
for row in range(n_rows):
    for col in range(n_lines):
        if np.isnan(loss_matrix[row, col]):
            ax3.text(col, row, 'n/c', ha='center', va='center',
                     fontsize=8, color='grey')
        else:
...
ax3.set_xticks(range(n_lines))
ax3.set_xticklabels([f'Line {i}' for i in range(n_lines)], fontsize=9)
ax3.set_yticks(range(n_rows))
ax3.set_yticklabels(row_labels, fontsize=8)
ax3.set_xlabel("Line", fontsize=10)
ax3.set_title("Active Power Losses per Line (MW) — OPF Economic Dispatch",
              fontsize=11, fontweight='bold')

cbar3 = fig3.colorbar(im3, ax=ax3, fraction=0.03, pad=0.02)
cbar3.set_label('Loss (MW)', fontsize=9)

plt.tight_layout()
plt.savefig(os.path.join(script_dir, 'task1_losses_heatmap.png'),
            dpi=150, bbox_inches='tight')
print("[Saved] task1_losses_heatmap.png")

plt.show()
print("\n[DONE] Heatmap figures saved.")